...
def encode_uniform(coef: list, n_cluster = 16):
    ...
    n_min = np.min(flat_coef)
    n_max = np.max(flat_coef)
    quantized = np.clip(np.round((flat_coef - n_min) / (n_max - n_min) * (n_cluster - 1)), 0, n_cluster - 1).astype(int)
    return quantized, n_min, n_max

def decode_uniform(quantized: np.ndarray, n_min: float, n_max: float, n_cluster: int,w: Wavelet, original_shape, level: int):
    flat_restored = (quantized * (n_max - n_min) / (n_cluster - 1)) + n_min
    coef_shapes = wavedec_periodic_dummy(original_shape, np.zeros_like(original_shape), w, level)
    restored = list()
    restored.append(flat_restored[:coef_shapes[0]] + 128)
    idx = coef_shapes[0]
